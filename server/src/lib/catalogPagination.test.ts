...
describe('PAGE_SIZE', () => {
  it('is fixed at 12 — DEC-107 (was §4a\'s 24; recorded deviation, user decision)', () => {
    expect(PAGE_SIZE).toBe(12)
  })
})

describe('computeCatalogPagination', () => {
  it('page 1 of a small result set: withinRange, skip 0, take 12', () => {
    expect(computeCatalogPagination(1, 6)).toEqual({ totalPages: 1, withinRange: true, skip: 0, take: 12 })
  })

  it('a later, in-range page skips by (page - 1) * 12', () => {
    expect(computeCatalogPagination(3, 100)).toEqual({ totalPages: 9, withinRange: true, skip: 24, take: 12 })
  })
  ...
  it('a past-the-end page is not withinRange and computes no skip', () => {
    // 6 total items, pageSize 12 -> totalPages 1; page 2 is past-the-end.
    const plan = computeCatalogPagination(2, 6)
    expect(plan.totalPages).toBe(1)
    expect(plan.withinRange).toBe(false)
    expect(plan.skip).toBeUndefined()
    expect(plan.take).toBe(12)
  })

  it('a page exactly equal to totalPages IS withinRange', () => {
    // 13 items -> totalPages 2. Page 2 is the real, final page.
    const plan = computeCatalogPagination(2, 13)
    expect(plan.withinRange).toBe(true)
    expect(plan.skip).toBe(12)
  })

  it('totalPages rounds up for a partial final page', () => {
    expect(computeCatalogPagination(1, 13).totalPages).toBe(2)
    expect(computeCatalogPagination(1, 12).totalPages).toBe(1)
    expect(computeCatalogPagination(1, 24).totalPages).toBe(2)
    expect(computeCatalogPagination(1, 25).totalPages).toBe(3)
  })

  it('take is always exactly PAGE_SIZE — pageSize is never client-influenced', () => {
    expect(computeCatalogPagination(1, 6).take).toBe(12)
    expect(computeCatalogPagination(5, 1000).take).toBe(12)
    expect(computeCatalogPagination(999, 6).take).toBe(12)
  })
