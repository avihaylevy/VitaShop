...
describe('GET /api/products — free-text search (Checkpoint E)', () => {
  ...
  it('direct field — English description match, a term absent from every name ("60 capsules" -> superherb-magnesium-max-550 only)', async () => {
  ...
  // wildcard-as-LIKE-meaning. None of the seed's text fields contain a
  // literal %, _, or backslash — verified by inspection above — so if
  // escaping ever silently stopped working (a bare % reverting to
  // match-all), these would start returning every active product instead of
  // zero.
  describe('literal-substring semantics — runtime proof against real PostgreSQL', () => {
    it('q="%" does not become match-all — zero results, not every product', async () => {
